...
// GET /api/geocode?q=<partial city name>
// Backs the search-bar autocomplete dropdown. Returns a light list of
// candidate places (name/state/country/lat/lon) — no weather call, no
// history logging — so it stays fast enough to run on every keystroke.
async function getGeocodeSuggestions(req, res) {
  const q = (req.query.q || "").trim();
  if (q.length < 2) {
    return res.status(200).json({ success: true, data: [] });
  }

  const raw = await owm.getGeocodeSuggestions(q, 6);

  // De-dupe near-identical entries (same name+country+state) that
  // OpenWeather sometimes returns for large cities.
  const seen = new Set();
  const results = raw
    .map((place) => ({
      name: place.name,
...
// Labels an arbitrary point on the map (e.g. the user clicking/tapping
// somewhere with no named marker) with its nearest place name.
async function getReverseGeocode(req, res) {
  const { lat, lon } = req.query;
  const raw = await owm.reverseGeocode({ lat, lon });
  const place = raw[0];
