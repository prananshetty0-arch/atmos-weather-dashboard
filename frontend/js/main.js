...
const state = {
  ...
  favorites: [],
  mapKey: "",
  localTimeTimer: null, // ticks the searched location's local clock every second
};
...
// ---------------------------------------------------------------------
async function loadWeatherFor({ city, lat, lon, onError }) {
  // setHeroLoading(true);
  // renderForecastSkeleton();
  ...
  try {
    ...
    // Restart the local-time ticker for the newly searched location —
    // only one interval ever runs at a time.
    renderLocalTime(weather);
    if (state.localTimeTimer) clearInterval(state.localTimeTimer);
    state.localTimeTimer = setInterval(() => renderLocalTime(weather), 1000);

    focusLocation(weather.lat, weather.lon, `${weather.city}${weather.country ? ", " + weather.country : ""}`);
    updateFullMapLink(weather);
...
function updateFullMapLink(weather) {
  const link = document.getElementById("open-full-map");
  if (!link) return;
  ...
  link.href = `map.html?${params.toString()}`;
}
...
function wireSearch() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    loadWeatherFor({
      ...
      onError: () => {
        ...
        void form.offsetWidth; // restart animation if the user retries quickly
  ...
  attachAutocomplete({
    ...
    dropdown,
    onSelect: (place) => {
      loadWeatherFor({ lat: place.lat, lon: place.lon, city: place.name });
    },
  });
}
...
// ---------------------------------------------------------------------
async function init() {
  // Safety net: never let the boot loader stay up forever if something
  // upstream (geolocation, network) stalls.
  setTimeout(() => document.getElementById("app-loader")?.classList.add("is-hidden"), 6000);
  ...
  const hideLoader = () => document.getElementById("app-loader")?.classList.add("is-hidden");
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
      (pos) => loadWeatherFor({ lat: pos.coords.latitude, lon: pos.coords.longitude }).finally(hideLoader),
      () => loadWeatherFor({ city: "London" }).finally(hideLoader),
      { timeout: 8000 }
    );
  } else {
    loadWeatherFor({ city: "London" }).finally(hideLoader);
