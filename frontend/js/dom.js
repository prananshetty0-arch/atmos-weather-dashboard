...
import { formatTemp, metersToKm, capitalize, timeAgo, localTimeInfo } from "./utils.js";
...
export const el = {
  ...
  localTimeClock: document.getElementById("local-time-clock"),
  localTimeDayNight: document.getElementById("local-time-daynight"),
  localTimeDate: document.getElementById("local-time-date"),
  localTimeTz: document.getElementById("local-time-tz"),
...
/** Renders (and can be re-invoked every second to tick) the searched
 *  location's local time, date, day/night status and UTC offset — all
 *  derived from the timezone offset the weather API returned. */
export function renderLocalTime(weather) {
  if (!el.localTimeClock) return;
  const info = localTimeInfo(weather?.timezoneOffset);
  if (!info) {
    el.localTimeClock.textContent = "--:--";
    el.localTimeDayNight.textContent = "";
    el.localTimeDate.textContent = "—";
    el.localTimeTz.textContent = "GMT--:--";
    return;
  }
  el.localTimeClock.textContent = info.time;
  el.localTimeDayNight.textContent = info.isDay ? "☀️" : "🌙";
  el.localTimeDate.textContent = info.date;
  el.localTimeTz.textContent = info.gmt;
}
